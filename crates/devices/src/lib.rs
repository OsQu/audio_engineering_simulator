...
//!
//! It sits **on** the engine and is consumed by both [`wasm-bindings`](../wasm_bindings/index.html)
//! (the browser) and the native `harness` (render scenarios), so none of it is JS-specific. serde
//! lives here (the IR + descriptors serialize); the `JsValue` bridge stays in `wasm-bindings`.

mod catalog;
mod scene;

pub use catalog::{
    DeviceDescriptor, ParamDescriptor, ParamKind, PortDescriptor, PortDirection, PortDomain,
    PortKind, build_node, descriptors,
};
