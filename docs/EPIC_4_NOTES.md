# Epic 4 — UI: Skeuomorphic Panels + Patch Cables — Design Notes & Delivery Record

Companion archive to `IMPLEMENTATION_PLAN.md`. Epic 4 is **substantially complete** — Stories 4.1–4.6 and
4.8 done (the engine→UI seam, skeuomorphic device panels, the spatial world of rooms/racks/walls, patch
cables → live graph mutation, visualization, and device focus mode); **Story 4.7 (scope + spectrum waveform
probes) is deferred**. The plan keeps a tight summary of Epic 4; **this file is the full record** — the
*settled design notes* (with the rejected alternatives and the reasoning that justified each choice), the
per-task breakdown, and the per-task *Delivered* notes.

Read this when a later epic's design decision turns on **why** Epic 4 was built the way it was, or when you
need the exact API/behavior of something Epic 4 shipped. For *what exists and what binds you going forward*,
the plan's Epic 4 summary is enough; come here for the depth behind it.

The plan's per-story section ordering (Goal → Watch out → Design notes → Tasks → Validate → Delivered) is
preserved. What follows is the Epic 4 section as it stood in the plan at close (the deferred Story 4.7 keeps
its settled sketch, for when it resumes).
...
#### Story 4.7 — Visualization, part 2: scope + spectrum (waveform probes) — ⏸️ **Deferred**

_Deferred at Epic 4 close (still coarse — to be planned when picked up)._ Split out of Story 4.5 at its
pickup: the **raw per-sample tap** a scope and spectrum need — a distinct mechanism from 4.5's scalar
readout lane. A **zero-copy sample ring** (à la `out_ptr`, tapping a node/port's block), a **scope**
rendering the waveform on a device screen / global tool, and an **FFT spectrum**. Builds on the 4.5 probe
addressing (`(device, probe id)` through `BuiltScene`) and the meter-screen UI. It is **independent of 4.8**
(no shared surface), which is why 4.8 was picked up first. _Open at pickup:_ the ring shape + who owns it
(engine-owned buffer vs. exposed pool lane); **FFT in the engine vs. JS** (a JS FFT keeps the engine lean;
an engine FFT keeps the DSP in one place — measure); tap cost on the hot path; which probes are
device-embedded vs. global. _Scope guard:_ waveform probes only — the scalar meters + analog-domain
readouts are 4.5.
