# Epic 1 — Headless Voltage Engine — Design Notes & Delivery Record

Companion archive to `IMPLEMENTATION_PLAN.md`. Epic 1 is **complete** (Stories 1.1–1.7, 229
engine tests green). The plan keeps a tight summary of Epic 1; **this file is the full record** —
the *settled design notes* (with the rejected alternatives and the reasoning that justified each
choice), the per-task breakdown, the hand-calc oracles, and the per-task *Delivered* notes.

Read this when a later epic's design decision turns on **why** Epic 1 was built the way it was, or
when you need the exact API/behavior of something Epic 1 shipped. For *what exists and what binds
you going forward*, the plan's Epic 1 summary is enough; come here for the depth behind it.
...
## Epic 1 framing (as originally written)
...
story validates real phenomena on real chains, not in isolation. Each story ends with an explicit
*Validate* gate — don't start the next story until it's green. Phenomena are validated in the story
where their prerequisites first exist, never batched at the end.
...
*Validate (met):* SNR-down-the-chain (1.4.1), DC removal (1.4.2), and clip-onset voltages (1.4.3) all match
